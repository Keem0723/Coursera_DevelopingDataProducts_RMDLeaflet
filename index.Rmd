...
```{r Data Manipulation & Visualization 1, message=FALSE, warning=FALSE}
...
# Joining the summary table & the lat-long data table together in order to get lat-long information in the same table & renaming columns
...
```{r Data Manipulation & Visualization 2, message=FALSE, warning=FALSE}
# Filtering data to only include active rockets, summarizing based on Country of Launch & whether or not the launch is commercial/private or is for the State (or Government)
sumDat2 <- spaceDat %>% 
        select(`Country of Launch`, `Private or State Run`) %>%
        group_by(`Country of Launch`) %>%
